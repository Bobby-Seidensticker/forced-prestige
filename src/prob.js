...
export function rand(min, max) { // INCLUSIVE ON BOTH SIDES
...
export function pyRand(min, max) { // inclusive of min, exclusive of max
  if (typeof (min) !== 'number' || typeof (max) !== 'number') {
...
export function rootRand(min, max) { // call
  // var root = 5;
...
export function pProb(lambda, limit) {
...
export function memoFact(count) {
  var arr = [ 1, 1 ];
...
/*
export function randColor(base, range) {
  var original = base;
  if (typeof (base) === 'string') {
    ...
    base = [
      parseInt(base.slice(0, 2), 16), parseInt(base.slice(2, 4), 16),
